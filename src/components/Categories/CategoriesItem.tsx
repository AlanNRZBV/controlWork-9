...
import { isCategoriesDeleting, isCategoriesLoading } from './categoriesSlice.ts';
import { Spinner } from 'react-bootstrap';
import { editCategory, toggleVisibility } from '../Modals/CategoryForm/categoryFormSlice.ts';
...
const CategoriesItem:FC<IApiCategory> = ({name, type,id}) => {
  ...
  return (
    <div className="border border-1 rounded-3 shadow-sm p-3 d-flex mb-3 align-items-center">
      ...
      <button onClick={onEdit} type="button" className="btn btn-outline-warning ms-5 me-3" disabled={isLoading || isDeleting}>Edit</button>
      <button onClick={onDelete} type="button" className="btn btn-danger" disabled={isDeleting}>
